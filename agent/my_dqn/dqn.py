...
class NetFighter(nn.Module):
    def __init__(self, n_actions):
        ...
        self.info_fc = nn.Sequential(
            nn.Linear(COURSE_NUM + 2 + COURSE_NUM + 1 + DIST_NUM + 1 + FIGHTER_NUM + 1, 256),
            nn.Tanh(),
        )
...
class RLFighter:
    def __init__(
        ...
        if self.gpu_enable:
            ...
            self.target_net.load_state_dict(torch.load('model/my_dqn/model.pkl'))
        else:
            self.target_net.load_state_dict(torch.load('model/my_dqn/model.pkl', map_location=lambda storage, loc: storage))
